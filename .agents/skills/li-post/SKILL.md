...
> OpenAI/Codex compatibility: when this skill refers to another `li-*` skill, invoke that sibling skill by name. Use `.linkedin-agent/` for persistent project-local state when a writable filesystem is available; otherwise use the current conversation/project context and do not claim persistence that did not occur.
...
1. Read `.linkedin-agent/voice.md` if it exists. That file is the user's
...
append the post to `.linkedin-agent/log.md` with the date, the hook used
